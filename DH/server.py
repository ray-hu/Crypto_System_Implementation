...
if __name__ == "__main__":
    ...
    p1,q1 = getPrime(key_size),5
    p2,q2 = getPrime(key_size),2
    
    print('Public knowledge for confidentiality: ')
    print(" "*5+'p = {}, g = {}.'.format(p1,q1))
    print('Public knowledge for integrity: ')
    print(" "*5+'p = {}, g = {}.'.format(p2,q2))
    ...
    gen_public_key(system_confi, private_key, public_key_path, '0')
    print("="*80)
    ...
    for i in range(num_clients):
        flag = str(input('Ready to receive public keys from client? y/n :'))
        receive_file(public_key_path, HOST, CLIENT_PORT[i])
        print(" " * 5 +"received the public key from ", (HOST,CLIENT_PORT[i]))
    else:
        print('Did not receive any public key!')
    ...
    str(input('Ready to broadcast your public keys? y/n :'))
    # broadcast public key files to clients
    #print(" " * 5 +'broadcast public keys to clients...')
    # revise the code here as broadcast
    broadcast(public_key_path, HOST, PORT, num_clients)
    print('Done!')
    print("="*80)
    ##########################################################################
    ...
    filePath = os.path.join(cwd, "04.pdf")
    str(input('Path of the file to be sent: "server/04.pdf"? y/n :'))
    ...
    iv = long_to_bytes(getRandomNBitInteger(system_confi['key_size']))
    public_key_list = eval(read_file(public_key_path, 'r'))
    private_key = eval(read_file(private_key_path, 'r'))
    
    # encrypt and send one by one
    for i in range(num_clients):
        public_key = public_key_list[str(i+1)]
        #print(public_key)
        key_c = long_to_bytes(pow(public_key[0], private_key['kc'], system_confi['p_g_c'][0]))
        key_i = long_to_bytes(pow(public_key[1], private_key['ki'], system_confi['p_g_i'][0]))
        #print(len(key_c), len(key_i), len(iv))
        ...
        cipher_message = {'ip':str(i+1), 'iv':iv, 'ciphertext': ciphertext, 'MAC': MAC, 'file_name': "04.pdf"}
        ...
        print(' '*5 + 'sending ciphertext to client {}...'.format(i+1))

        cipher_message = eval(read_file(cipherPath, 'r'))
        #print(cipher_message)
        send_file(cipherPath, HOST, PORT)
    print("Done!")
    print('='*80)
